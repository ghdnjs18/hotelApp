...
const Search = () => {
    ...
    const changeCaptionHighlight = (e) => {
        ...
        if (e.keyCode === 13) { // 인덱스 값을 증가시켜 하이라이트조절은 한다.
            const target = document.getElementById(index)
            setDestination(target.innerText) // 선택된 태그의 문자만 가져온다 
            setOpen('hide')
            // setIndex(0)
        } else if (e.keyCode === 40) { // 인덱스 값을 증가시켜 하이라이트조절은 한다.
        ...
        <div className="Search-container">
            <div className="Search-inputs">
                <div className="destination-container">
                    <Input name='destination' type='text' placeholder='목적지를 입력하세요...' width='large' value={destination} onChange={handleChange} onKeyUp={changeCaptionHighlight}/>
                ...
                <Input name='adults-number' type='number' placeholder='안원수' width='middle' min={1} max={7} value={adultsNumber} onChange={handleChange}/>
